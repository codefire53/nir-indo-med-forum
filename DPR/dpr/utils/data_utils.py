...
class TokenIdsCache:
    def __init__(self, data_dir: str, name: str):
        with open(f"{data_dir}/{name}_meta") as meta_json:
            metadata = json.load(meta_json)
        self.n_rows = metadata['total_number']
        self.max_seq_length = metadata['embedding_dim']
        self.ids_arr = np.memmap(f"{data_dir}/{name}.memmap", 
                        shape=(self.n_rows, self.max_seq_length),
                        dtype=np.dtype(metadata['dtype']))
        self.lengths_arr = np.load(f"{data_dir}/{name}_length.npy") #maximum length for each item
        assert len(self.lengths_arr) == self.n_rows
...
class SequenceDataset(Dataset):
    def __init__(self, token_ids_cache: TokenIdsCache, max_seq_length: int):
        self.token_ids_cache = token_ids_cache
        self.max_seq_length = max_seq_length

    def __len__(self):
        return len(self.token_ids_cache)

    def __getitem__(self, item):
        id = item
        input_ids = self.token_ids_cache[item].tolist()
        seq_length = min(len(input_ids)-1, self.max_seq_length-1)
...
class SubsetSeqDataset:
    def __init__(self, subset: List[int], token_ids_cache: TokenIdsCache, max_seq_length: int):
        self.subset = sorted(list(subset))
        self.full_dataset = SequenceDataset(token_ids_cache, max_seq_length)
    
    def __len__(self):
        return len(self.subset)
    
    def __getitem__(self, item):
        return self.full_dataset[self.subset[item]]
