...
from .biencoder import BiEncoder, BiEncoder_InBatch
...
def get_bert_inbatch_biencoder_components(args, inference_only: bool = False, **kwargs):
    dropout = args.dropout if hasattr(args, 'dropout') else 0.0
    question_encoder = HFBertEncoder.init_encoder(args.pretrained_model_cfg,
                                                  projection_dim=args.projection_dim, dropout=dropout, **kwargs)
    ctx_encoder = HFBertEncoder.init_encoder(args.pretrained_model_cfg,
    ...
    biencoder = BiEncoder_InBatch(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder)
...
def get_bert_tensorizer(args, tokenizer=None):
    if not tokenizer:
        tokenizer = AutoTokenizer.from_pretrained(args.pretrained_model_cfg, do_lower_case=args.do_lower_case, use_fast=False)
...
class HFBertEncoder(BertModel):
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        ...
        if transformers.__version__.startswith("4") and isinstance(
            ...
            hidden_states = None
            out = super().forward(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,)
